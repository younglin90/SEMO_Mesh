...
double SEMO_Solvers_Builder::calcCompCoupledEq(
	...
	for(int i=0; i<mesh.faces.size(); ++i){
		...
		double diffPDU = 0.0;//0.5*Mdash*PLP*PRM*0.5/Cbar;
		...
		A_vals[id] += ( RhoF * WUL * nvec[0] * UF * area + weightL * RhoL * UnF * area + diff_viscous * area + 
						diffPDU*nvec[0] * nvec[0] * area );
		A_rows.push_back(i_glo); A_cols.push_back(j_glo);
		A_vals.push_back(( RhoF * WUR * nvec[0] * UF * area + weightR * RhoR * UnF * area - diff_viscous * area - 
						diffPDU*nvec[0] * nvec[0] * area ));
		...
		if(face.getType() == SEMO_Types::INTERNAL_FACE) {
			...
			A_vals[id] -= ( RhoF * WUR * nvec[0] * UF * area + weightR * RhoR * UnF * area - diff_viscous * area - 
						diffPDU*nvec[0] * nvec[0] * area  );
			A_rows.push_back(i_glo); A_cols.push_back(j_glo);
			A_vals.push_back(-( RhoF * WUL * nvec[0] * UF * area + weightL * RhoL * UnF * area + diff_viscous * area + 
						diffPDU*nvec[0] * nvec[0] * area  ));
	...
	for(auto& boundary : mesh.boundary){
		...
		if(boundary.neighbProcNo == -1){
			...
			for(int i=str; i<end; ++i){
				...
				A_vals[id] += coeffP_diff * ( RhoF * tmp1 / dPN * MFF[0] * area );
